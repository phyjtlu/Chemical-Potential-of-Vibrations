...
%&=-\frac{1}{\hbar\omega}\int \frac{d\varepsilon_\alpha}{2\pi} {\rm tr}[M^m A_\beta(\varepsilon-\hbar\omega)M^n A_\alpha(\varepsilon_\alpha)] \nonumber\\
%&\times (n_F(\varepsilon_\alpha-\mu_\alpha,T_\alpha)-n_F(\varepsilon_\beta-\mu_\beta,T_\beta)).
\label{eq:gamma}
\end{align}
...
\emph{Non-reciprocal energy transport.--}
...
\end{figure}
\begin{figure}[h]
	\includegraphics[scale=0.3,angle=-90]{coldspot.pdf}
