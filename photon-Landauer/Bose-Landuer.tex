...
%\title{Nonequilibrium reservoir engineering of a biased coherent conductor for hybrid energy transport}
...
\begin{figure}[h]
	...
	\caption{Two limiting cases of electronic heating (a) and cooling (b). In (a), we have a filled electronic level $\varepsilon_L$ that couples to the left electrode with chemical potential $\mu_L$, and an empty level $\varepsilon_R$ that couples to the right electrode with chemical potential $\mu_R$. We have $\mu_L>\mu_R$. Heating of the bosonic mode is due to resonant recombination of inter-electrode (process 4 in Fig.~\ref{fig:ehp}). In (b), the situation is reversed. The left state $\varepsilon_L$ is empty, while the right state $\varepsilon_R$ is filled. When $\mu_R>\mu_L$, the electrical current can be used to cool the bosonic mode through creation of inter-electrode EHPs (process 4' in \ref{fig:ehp}). }
	\label{fig:resonant}
\end{figure}
